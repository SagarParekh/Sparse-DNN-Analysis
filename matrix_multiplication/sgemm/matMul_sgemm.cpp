...
int main() {
    ...
    cout <<"\n This example computes real matrix C=alpha*A*B+beta*C using "<<endl<<" Intel(R) MKL function sgemm, where A, B, and  C are matrices and "<<endl<<" alpha and beta are double precision scalars\n"<<endl;
   
    double input[100];
    int x=0;
    fstream textfile;
    textfile.open("input_sgemm.txt");
    ...
    A = new float[m*p*sizeof( double )];
    B = new float[p*n*sizeof( double )];
    C = new float[m*n*sizeof( double )];
    temp = new float[m*n*sizeof( double )];
